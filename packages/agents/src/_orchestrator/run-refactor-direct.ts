...
import { type RefactorRepositoryEvent } from './refactor-repository.js';

/**
 * Direct (non-Inngest) pipeline runner for refactor jobs.
 ...
 */
export async function runRefactorDirect(
  parsed: RefactorRepositoryEvent,
  databaseUrl: string,
): Promise<void> {
  // 0. workflow_started
  {
  ...
  // 3. cartograph — refactor always uses planFromSource with sourceKind='refactor-intent'
